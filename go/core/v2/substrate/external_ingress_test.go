...
const (
	externalRuntimeTestActorUID   = "00000000-0000-4000-8000-000000000401"
	externalRuntimeTestInstanceID = "8bd650a8-9775-488f-8bc1-0d52bf7bdcab"
)

type externalRuntimeRevisionStore struct {
	revision *dbpkg.RuntimeRevision
}

func (s *externalRuntimeRevisionStore) GetRuntimeRevision(context.Context, string) (*dbpkg.RuntimeRevision, error) {
...
func TestOpenActorIngressFencesIdentityAndBridgesBytes(t *testing.T) {
	...
	if open.GetActor().GetAtespace() != instance.GetNamespace() || open.GetActor().GetName() != actorName(instance.GetId()) || open.GetActorUid() != externalRuntimeTestActorUID {
	...
	if request := api.request; request.GetActor().GetAtespace() != instance.GetNamespace() || request.GetActor().GetName() != actorName(instance.GetId()) {
...
func TestRuntimePlacementRequiresExactPersistedRevision(t *testing.T) {
	...
	store := &externalRuntimeRevisionStore{revision: revision}
	dialer := &Connector{revisions: store}
...
func externalRuntimeTestIdentity() (*apiv1alpha1.AgentInstance, *dbpkg.RuntimeRevision, *ateapipb.Actor) {
	instance := &apiv1alpha1.AgentInstance{
		Id: externalRuntimeTestInstanceID, Namespace: "team-a", PreparedRevision: "revision-external", A2AAuthority: "runtime.internal",
	}
	revision := &dbpkg.RuntimeRevision{
	...
	actor := &ateapipb.Actor{
		Metadata: &ateapipb.ResourceMetadata{
			Atespace: instance.GetNamespace(), Name: actorName(instance.GetId()), Uid: externalRuntimeTestActorUID,
		},
