...
func (w *RuntimeWorkflow) Quiesce(ctx context.Context, instance *apiv1alpha1.AgentInstance) (*dbpkg.AgentInstanceTaskSnapshot, error) {
	snapshot, err := w.runtime.Quiesce(ctx, instance)
	if err != nil {
		return nil, fmt.Errorf("quiesce AgentInstance runtime: %w", err)
	}
	return snapshot, nil
}
...
func (w *RuntimeWorkflow) Fork(ctx context.Context, instance *apiv1alpha1.AgentInstance, checkpoint *dbpkg.AgentInstanceCheckpoint) (*apiv1alpha1.AgentInstance, error) {
	if instance.GetState() == apiv1alpha1.AgentInstanceState_AGENT_INSTANCE_STATE_READY {
		return instance, nil
	}
	if instance.GetState() != apiv1alpha1.AgentInstanceState_AGENT_INSTANCE_STATE_CREATING {
	...
	endpoint, err := w.runtime.Fork(ctx, instance, checkpoint)
	if err != nil {
		return nil, fmt.Errorf("fork AgentInstance runtime: %w", err)
	}
	if endpoint.A2AAuthority == "" {
		return nil, fmt.Errorf("fork AgentInstance runtime: A2A authority is empty")
	}
	instance, err = w.store.MarkAgentInstanceReady(ctx, instance.GetId(), endpoint.A2AAuthority)
	if err != nil {
...
func (w *RuntimeWorkflow) claim(
	...
	// the runtime. This lets every API replica reject a different mutation while
