...
func TestParsePortableExternalUsesSameStandaloneOCISkillContract(t *testing.T) {
	raw := []byte(`{"version":"v2","runtime":"claude","root":{"templateName":"agent-one","instruction":"help","model":{"provider":"Anthropic","name":"claude-sonnet-4-6"},"skills":[{"name":"review","source":{"oci":"ghcr.io/acme/claude/skills/review@sha256:` + strings.Repeat("a", 64) + `"}}]}}`)
	cfg, err := ParsePortableExternal(raw, "ghcr.io/acme/claude/skills/")
	if err != nil {
		t.Fatal(err)
	}
	if cfg.Model != "claude-sonnet-4-6" || cfg.AppendSystemPrompt != "help" || cfg.SkillResources == nil ||
		len(cfg.SkillResources.Skills) != 1 || cfg.SkillResources.Skills[0].Name != "review" {
		t.Fatalf("portable Claude config = %#v", cfg)
	}
	if _, err := ParsePortableExternal(raw, "ghcr.io/acme/sibling/skills/"); err == nil || !strings.Contains(err.Error(), "not allowed") {
		t.Fatalf("ParsePortableExternal() foreign repository error = %v", err)
	}
}
...
func TestAgentsJSON(t *testing.T) {
	...
	parsed, err := Parse([]byte(`{"version":3,"claude_executable":"claude","expected_claude_version":"2.1.236","strict_version":true,"agents":` + raw + `,"max_event_bytes":100,"max_stderr_bytes":100,"interrupt_grace_millis":100}`))
	if err != nil {
		t.Fatal(err)
	}
...
func TestParseValidates(t *testing.T) {
	contents := `{"version":3,"claude_executable":"claude","expected_claude_version":"2.1.236","strict_version":true,"model":"claude-test","append_system_prompt":"help","max_event_bytes":100,"max_stderr_bytes":100,"interrupt_grace_millis":100}`
