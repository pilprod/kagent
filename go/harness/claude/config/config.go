...
// Agent is one compiler-owned local Claude subagent materialized as an
// invocation-scoped plugin so its prompt never crosses the process argv boundary.
...
// ParsePortableExternal translates the shared placement-neutral coding-agent
// contract into the pinned Claude driver configuration. Standalone skills are
// accepted only below the exact OCI repository prefix selected by local owner policy.
func ParsePortableExternal(contents []byte, allowedSkillRepositoryPrefix string) (Config, error) {
	portable, err := codingagent.Decode(contents)
	if err != nil {
		return Config{}, fmt.Errorf("decode portable external-host config: %w", err)
	}
	if portable.Runtime != codingagent.RuntimeClaude {
		return Config{}, fmt.Errorf("portable external-host config selects runtime %q, want %q", portable.Runtime, codingagent.RuntimeClaude)
	}
	resources, err := codingagent.ExternalHostSkillResources(portable.Root, allowedSkillRepositoryPrefix)
	if err != nil {
		return Config{}, fmt.Errorf("validate portable Claude resources: %w", err)
	}
	translated := Production(portable.Root.Model.Name, portable.Root.Instruction)
