 ...
 * the target.
 */
export const EXTENSION_TABLE_IDS = [
...
export type ExtensionTableId = (typeof EXTENSION_TABLE_IDS)[number];

const EXTENSION_TABLE_ID_SET: ReadonlySet<string> = new Set(EXTENSION_TABLE_IDS);

export function isExtensionTableId(value: string): value is ExtensionTableId {
  return EXTENSION_TABLE_ID_SET.has(value);
}
 ...
 */
export interface ExtensionTableColumn<TRow = never> {
 ...
 */
export function defineExtensionTableColumn<TRow>(
  column: ExtensionTableColumn<TRow>,
): ExtensionTableColumn<TRow> {
  return column;
}

/** Contributions for one table, in declaration order. */
export function extensionColumnsForTable<TRow>(
  columns: readonly ExtensionTableColumn<never>[] | undefined,
  tableId: ExtensionTableId,
): ExtensionTableColumn<TRow>[] {
  return (columns ?? []).filter(
    (column) => column.tableId === tableId,
  ) as unknown as ExtensionTableColumn<TRow>[];
}
 ...
 */
export function withExtensionColumns<TRow, TColumn extends ColumnLike>(
  coreColumns: readonly TColumn[],
  contributed: readonly ExtensionTableColumn<TRow>[],
): TColumn[] {
  if (contributed.length === 0) return [...coreColumns];

  const asColumn = (column: ExtensionTableColumn<TRow>) =>
    ({
      key: column.id,
      title: column.title,
      width: column.width,
